...
def upsert_node(tx, node):
    ...
    print(f"Creating node: {node['id']} with labels {labels} and props {props}")  # Debug print
    tx.run(
...
def create_relationship(tx, source_id, rel):
    ...
    if not target_id:
        return
    print(f"Creating relationship: {source_id} -[{rel_type}]-> {target_id}")  # Debug print
    cypher = (
